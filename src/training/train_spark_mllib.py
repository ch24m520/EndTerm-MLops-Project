...
def ensure_features_and_label(df):
    """
    Ensures the Spark DataFrame has:
      - a 'label' column (renamed from 'Survived' if needed)
      - a 'features' column assembled from numeric + OHE categorical columns
    """
    ...
    if "Fare" in df.columns:
        num_cols.append("Fare")

    # Family features if present
    ...
    if "Pclass" in df.columns:
        cat_cols.append("Pclass")  # treat as categorical
...
def main(cfg_path: str):
    ...
    # Load dataset and ensure required columns
    ...
    for col in ["features", "label"]:
        if col not in df.columns:
            raise ValueError(
                f"Required column '{col}' missing after preprocessing fallback. "
    ...
    # ---------- MLflow tracking: respect env var if provided ----------
    ...
    with mlflow.start_run() as run:
        ...
        # Store run_id for later
        Path(out_dir, "run_id.txt").write_text(run.info.run_id)
