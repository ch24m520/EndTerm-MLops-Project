...
def main(cfg_path):
    ...
    # Expect columns: features (Vector), label (numeric)
    train, test = df.randomSplit([1.0 - C["train"]["test_size"], C["train"]["test_size"]],
                                 seed=C["train"]["seed"])

    # Model & CV
    lr = LogisticRegression(featuresCol="features", labelCol="label",
                            maxIter=C["model"]["maxIter"])
...
# Confusion matrix (2x2)
        ...
        # Coefficients (optional, for LR)
        coef_path = os.path.join(out_dir, "coefficients.txt")
        with open(coef_path, "w") as f:
            f.write(str(best.coefficients))

        # Save simple metrics JSON (also as DVC-tracked artifact)
...
# MLflow logging
        ...
        # Log model (no registry yet; that’s Task 3)
        mlflow.spark.log_model(model, artifact_path="model")

        # Save run_id for reference
